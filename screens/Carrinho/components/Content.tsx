...
export default function Content(props: any) {
  ...
  useEffect(() => {
    fetch(`${ipnode}/api/usuarios/carrinho/${obterId}`)
        ...
        <View>
          <ScrollView horizontal={false}>
            {produtos.map((itens, ix) => (
              <View key={ix} style={{ flex: 1, justifyContent: "center", padding: 10, borderBottomWidth: 1, borderColor: "silver", flexDirection: "row" }}>
                
                <View style={{flexDirection:"column", flex: 1}}>
                <Text style={styles.txtproduto}> Produto: {itens.nomeproduto}</Text>
...
async function obterId(){
  ...
  await db.transaction((ds)=>{
    ds.executeSql(
      `select idusuario from dados order by id desc`,[],(_,{rows})=>{
